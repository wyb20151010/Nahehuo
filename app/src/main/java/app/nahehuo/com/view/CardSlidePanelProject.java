...
public class CardSlidePanelProject extends ViewGroup {
    private List<CardItemViewProject> viewList = new ArrayList<CardItemViewProject>();
    // 存放的是每一层的view，从顶到底
    ...
    private MyFlingCardListener mMyFlingCardListener;
    private Object obj1 = new Object();

    private CardSwitchListener cardSwitchListener; // 回调接口
    private List<ProjectListDict> dataList; // 存储的数据链表
    private int isShowing = 0; // 当前正在显示的小项
    private View leftBtn, rightBtn;
    private boolean btnLock = false;
    private GestureDetectorCompat moveDetector;
    private OnClickListener btnListener;
    private CardSwipeListenter mCardSwipeListenter;


    public CardSlidePanelProject(Context context) {
        this(context, null);
    }


    public CardSlidePanelProject(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }


    public CardSlidePanelProject(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.card);

        bottomMarginTop = (int) a.getDimension(R.styleable.card_bottomMarginTop,
                bottomMarginTop);
        yOffsetStep = (int) a.getDimension(R.styleable.card_yOffsetStep,
                yOffsetStep);
        // 滑动相关类
        mDragHelper = ViewDragHelper.create(this, 10f,
    ...
    @Override protected void onFinishInflate() {
        ...
        for (int i = num - 1; i >= 0; i--) {
            ...
            CardItemViewProject viewItem = (CardItemViewProject) childView;
            viewItem.setTag(i + 1);
 ...
 /*   private void initBottomLayout() {
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        @Override public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (child == bottomLayout || dataList == null ||
                    dataList.size() == 0 ||
    ...
    private void orderViewStack() {
        synchronized (obj1) {
            ...
            CardItemViewProject changedView = (CardItemViewProject) releasedViewList.get(0);
            ...
            changedView.offsetLeftAndRight(
                    initCenterViewX - changedView.getLeft());
            changedView.offsetTopAndBottom(
                    initCenterViewY - changedView.getTop() + yOffsetStep * 2);
            ...
            if (newIndex < dataList.size()) {
                ProjectListDict dataItem = dataList.get(newIndex);
            ...
            if (isShowing == dataList.size()&&null != cardSwitchListener) {
                cardSwitchListener.onFinished();
            }
        }
    }
    ...
    private void ajustLinkageViewItem(View changedView, float rate, int index) {
        ...
        ajustView.offsetTopAndBottom(
                offset - ajustView.getTop() + initCenterViewY);
    ...
    private void vanishOnBtnClick(int type) {
        synchronized (obj1) {
            ...
            if (finalX != 0) {
                ...
                if (mDragHelper.smoothSlideViewTo(animateView, finalX,
                        initCenterViewY + allHeight)) {
                    ViewCompat.postInvalidateOnAnimation(this);
                }
            }
    ...
    @Override public void computeScroll() {
        if (mDragHelper.continueSettling(true)) {
            ...
            // 动画结束
            synchronized (this) {
                if (mDragHelper.getViewDragState() ==
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            viewItem.layout(left, getPaddingTop(),
                    left + viewItem.getMeasuredWidth(),
        ...
        if (null != bottomLayout) {
            int layoutTop = viewList.get(0).getMeasuredHeight() +
                    bottomMarginTop;
            bottomLayout.layout(left, layoutTop, right,
                    layoutTop + bottomLayout.getMeasuredHeight());
        }
     ...
     * @param dataList 数据
     */
    public void fillData(List<ProjectListDict> dataList) {
        this.dataList = dataList;

        int num = viewList.size();
        for (int i = 0; i < num; i++) {
            CardItemViewProject itemView = (CardItemViewProject) viewList.get(i);
    ...
    public List<CardItemViewProject> getSelectedViews() {
        return viewList;
    }
     ...
     */
    public void appendData(List<ProjectListDict> appendList) {
        dataList.addAll(appendList);

        int currentIndex = isShowing;
        int num = viewList.size();
        for (int i = 0; i < num; i++) {
            CardItemViewProject itemView = viewList.get(i);
            itemView.setVisibility(View.VISIBLE);
    ...
    public void setCardSwipeListenter(CardSwipeListenter cardSwipeListenter) {
        mCardSwipeListenter = cardSwipeListenter;
    }
    ...
    public interface CardSwitchListener {
         ...
         * @param type 飞向哪一侧{@link #VANISH_TYPE_LEFT}或{@link
         * #VANISH_TYPE_RIGHT}
         */
    ...
    public interface CardSwipeListenter {
