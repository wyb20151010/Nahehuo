...
public class CardSlidePanel extends ViewGroup {
    ...
    private List<RecomJob> dataList; // 存储的数据链表
    ...
    @Override protected void onFinishInflate() {
        ...
        for (int i = num - 1; i >= 0; i--) {
            ...
            viewItem.setTag(i + 1);
            viewItem.iv_title.setOnClickListener(btnListener);
 ...
 /*   private void initBottomLayout() {
    ...
    private void orderViewStack() {
        synchronized (obj1) {
            ...
            changedView.findViewById(R.id.ll_content).setAlpha(1f);
            changedView.findViewById(R.id.iv_notlike).setAlpha(0f);
            changedView.findViewById(R.id.iv_like).setAlpha(0f);
            ...
            if (newIndex < dataList.size()) {
                RecomJob dataItem = dataList.get(newIndex);
    ...
    private void processLinkageView(View changedView) {
        ...
        int alllength;
        if (changeViewLeft < 0) {
            alllength = Math.abs(changeViewLeft) + 54;
            mCardSwipeListenter.onScroll(-alllength / 300f);
        }
        else if (changeViewLeft >= 0 && changeViewLeft < 54) {
            alllength = Math.abs(changeViewLeft);
            mCardSwipeListenter.onScroll(-alllength / 300f);
        }
        else if (changeViewLeft > 54) {
            alllength = Math.abs(changeViewLeft - 54);
            mCardSwipeListenter.onScroll(alllength / 300f);
        }
        else if (changeViewLeft == 54) {
     ...
     * @param dataList 数据
     */
    public void fillData(List<RecomJob> dataList) {
    ...
    public List<CardItemView> getSelectedViews() {
        return viewList;
    }
     ...
     */
    public void appendData(List<RecomJob> appendList) {
